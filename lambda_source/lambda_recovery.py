...
def lambda_handler(event, context):
  ...
  if action == action_process_prefixes:
    ...
    try:
      prefix_event_type = json_object['prefixEventType']
      if not prefix_event_type:
        raise KeyError
    except KeyError:
      msg = "Error: prefixEventType must be sent in request payload"
      logger.error(msg)
      return msg
    ...
    try:
      ...
      msg = "Error: prefixDate must be sent in request payload"
      logger.error(msg)
      return msg

    try:
      datetime.strptime(prefix_date, '%Y-%m-%d')
    except ValueError:
      msg = "Error: Incorrect format for 'prefixDate'. Expected 'yyyy-mm-dd' format"
      logger.error(msg)
      return msg
    ...
    try:
      ...
      logger.info("startPrefixNumber and endPrefixNumber not defined in request. "
                  "all data inside of prefixDate will be processed")
    ...
    if start_prefix or end_prefix:
      try:
        ...
        msg = "Error: 'startPrefixNumber' and 'endPrefixNumber' must be integer"
        logger.error(msg)
        return msg
      if start_prefix > end_prefix:
        msg = "Error: 'startPrefixNumber' must be less then 'endPrefixNumber'"
        logger.error(msg)
        return msg
    ...
    for prefix in prefixes:
      if start_prefix and end_prefix:
        prefix_path = historical_recovery_path + "/" + prefix_event_type + "/" + prefix_date + "/" + str(prefix)
      else:
        prefix_path = historical_recovery_path + "/" + prefix_event_type + "/" + prefix_date
    ...
    while stop_processing == False:
      ...
      for content in contents:
        recovery_key = content['Key']
        data_response = s3.get_object(Bucket=bucket, Key=recovery_key)
        response_file_key = data_response['Body'].read().decode('utf-8')
        logger.info('Send for processing response_file_key {} extracted from body of recovery key {}'
                    .format(response_file_key, recovery_key))

        is_recovery_key_removed = process_response_file(response_file_key, recovery_key)
        if is_recovery_key_removed == False:
          last_verified_key = recovery_key
...
def process_response_file(response_file_key, recovery_file_key):
  # found1 = key.find("11111111.txt")
  # found2 = key.find("333333333.txt")
  # if found1 > 0 or found2 > 0:
  bucket = os.environ["BUCKET_NAME"]
  s3.delete_object(Bucket=bucket, Key=recovery_file_key)
  logger.info('Deleted recovery obj {}'.format(recovery_file_key))
  return True
# else:
